...
import (
	...
	"github.com/teddy/sign-in-on/sqlStore"
	"github.com/teddy/sign-in-on/utils"
)
// 判断钱包地址是否存在 ； 判断矿机是否已经被绑定 ；绑定钱包地址； 绑定矿机

func InitBinder() {
	l4g.Debug(utils.T("api.binder.init.debug"))

	BaseRoutes.Workers.Handle("/worker_id/exist", ApiHandler(isEmailExist)).Methods("POST")
