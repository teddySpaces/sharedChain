...
func completeOAuth(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	uri := c.GetSiteURLHeader() + "/capp/signup/" + service + "/complete"
	...
	if exist {
		...
		redirectUrl = utils.GetSiteURL() + "/capp"
	} else {
		if state, err := app.SetCookieAndToken(w, r, user); err != nil {
			c.Err = err
			return
		} else {
			redirectUrl = utils.GetSiteURL() + "/capp/wechat?state=" + state
		}
	}
...
func completeOAuthMobile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	capp := utils.Cfg.GetCappService(model.SERVICE_WEIXIN)
	if capp == nil {
    ...
    userApiEndpoint := capp.UserApiEndpoint + "?access_token=" + accessToken + "&openid=" + openId
