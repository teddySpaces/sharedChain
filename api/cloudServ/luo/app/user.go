...
func SendSmsCode(mobile string, w http.ResponseWriter, r *http.Request) *model.AppError {
	capp := utils.Cfg.GetSmsService()
	if capp != nil && !capp.Enable {
	...
	if capp.Provider == "aliyun" {
		//阿里云-云通信
		templateParam := `{"code":"` + verificationCode + `"}`
		smsClient := NewALiYunSmsClient(capp.GatewayUrl)
		if err := smsClient.Execute(capp.AccessKeyId, capp.AccessKeySecret, mobile, capp.SignName,
			capp.TemplateCode, templateParam); err != nil {
